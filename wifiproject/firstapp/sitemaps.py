...
class PostSitemap(Sitemap):
    ...
    def items(self):
        ...
        merged = list(jobs) + list(results) + list(admits) + list(updates)
    ...
    def items(self):
        items = list(Job.objects.all()) + list(Result.objects.all()) + \
            list(Admitcard.objects.all()) + list(Govtupdate.objects.all())
    ...
    def location(self, obj):
        ...
        base_map = {"job": "job", "result": "result",
                    "admitcard": "admit-card", "govtupdate": "govt-update"}
        base = base_map.get(model, model)
        if slug:
            return f"/{base}/{slug}/"
