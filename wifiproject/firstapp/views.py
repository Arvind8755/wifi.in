...
def careerpower(request):
    ...
    # ✅ sirf teen arguments
    return render(request, "jobs/careerpower.html", context)
...
def free_job_alert_2024(request):
    jobs = Job.objects.all().order_by("-published_date")[:15]        
    ...
    # ✅ sirf teen arguments
    return render(request, "jobs/free-job-alert-2024.html", context)
...
def latestsarkariresult(request):
    jobs = Job.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:10]
    results = Result.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:10]
    admitcards = Admitcard.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:10]
    govtupdates = Govtupdate.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:10]
    ...
    return render (request, "results/latestsarkariresult.html", context)
...
def fast(request):
    ...
    # ✅ sirf teen arguments
    return render(request, "jobs/fast.html", context)


def wifiresultonline(request):
    ...
    # ✅ sirf teen arguments
    return render(request, "jobs/wifiresultonline.html", context)

def careers(request):
    jobs = Job.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:5]
    results = Result.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:30]
    admitcards = Admitcard.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:5]
    govtupdates = Govtupdate.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:10]

    paginator = Paginator(results, 30, orphans=1) # प्रति पेज 10 jobs दिखाने के लिए (2 बहुत कम है)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    # 2) chain se sabko jod do
    mixed_objects = list(chain(results, jobs, admitcards, govtupdates ))
    # 3) sabko created_at ke hisaab se sort kar do (naya sabse upar)
    mixed_objects = sorted(
        mixed_objects,
        key=lambda obj: obj.published_date,
...
def careerwill(request):
    ...
    # ✅ sirf teen arguments
    return render(request, "jobs/careerwill.html", context)
...
def recruitment(request):
    ...
    return render(request, "jobs/recruitment.html", {"latest_results": latest_results, "latest_admitcards": latest_admitcards, 'latest_govtupdates': latest_govtupdates, "page_obj": page_obj, "latest_posts": latest_posts,})
...
def latestresult(request):
  ...
  mixed_objects = sorted(
    ...
    # 4) agar 20 ya 30 hi dikhane hain:
  latest_posts = mixed_objects[:10]
  return render(request, 'results/latestresult.html',{ "results": results, "latest_jobs": latest_jobs, "latest_admitcards": latest_admitcards, 'latest_govtupdates': latest_govtupdates, "page_obj":page_obj, "latest_posts": latest_posts,})
...
def wifiresult(request):
  ...
  updates = Govtupdate.objects.all()
    # 2) chain se sabko jod do
  mixed_objects = list(chain(jobs, results, admitcards, updates))
    # 3) sabko created_at ke hisaab se sort kar do (naya sabse upar)
  mixed_objects = sorted(
        mixed_objects,
        key=lambda obj: obj.published_date,
    ...
    # 4) agar 20 ya 30 hi dikhane hain:
  latest_posts = mixed_objects[:12]
  return render(request, 'results/wifiresult.html',{ "results": results, "latest_jobs": latest_jobs, "latest_admitcards": latest_admitcards, 'latest_govtupdates': latest_govtupdates, "page_obj":page_obj, "latest_posts": latest_posts,})
...
def ncvtmis(request):
  ...
  return render(request, 'results/ncvtmis.html',  {"posts": qs, "latest_jobs": latest_jobs, "latest_admitcards": latest_admitcards, 'latest_govtupdates': latest_govtupdates, "page_obj":page_obj,"latest_posts": jobs_itiresult,})
...
def scvt(request):
  ...
  return render(request, 'results/scvt.html',  {"posts": qs, "latest_jobs": latest_jobs, "latest_admitcards": latest_admitcards, 'latest_govtupdates': latest_govtupdates, "page_obj":page_obj,"latest_posts": jobs_itiresult,})
...
def rohan(request):
  ...
  paginator = Paginator(qs, 21, orphans=1) 
  page_number = request.GET.get('page')
  page_obj = paginator.get_page(page_number)
...
def neha(request):
  ...
  paginator = Paginator(qs, 20, orphans=1) 
  page_number = request.GET.get('page')
  page_obj = paginator.get_page(page_number)
...
def latestnews(request):
    ...
    def collect(qs, label):
        for obj in qs:
            dt = _pick_dt(obj)  # may be None if nothing found
            thumb = _thumb_url(request, obj) or PLACEHOLDER
            dt_display = timezone.localtime(dt).strftime("%d %b %Y") if dt else ""
            items.append({
                "title": _title(obj),
                "url": _url(obj),
                "thumb": thumb,
    ...
    collect(Job.objects.all(), "Jobs")
    collect(Result.objects.all(), "Results")
    collect(Admitcard.objects.all(), "Admit Cards")
    collect(Govtupdate.objects.all(), "Govt Updates")

    # Items with no dt should go last
    items.sort(key=lambda x: (x["dt"] is None, x["dt"]), reverse=True)
...
def sarkariresultnewupdate(request):
    items = []
    ...
    page_obj = Paginator(items, 35).get_page(request.GET.get("page"))
...
def blog(request):
    ...
    def collect(qs, label):
        for obj in qs:
            ...
            if dt:
                ...
                dt_display = dt_local.strftime("%d %b %Y")
                dt_iso = dt_local.isoformat()
            else:
                dt_display = ""
                dt_iso = ""

            url, debug_reason = _url_for_type(label, obj)
            ...
            items.append({
                ...
                "alt": f"{label} - thumbnail",
                "slug": _slug(obj),
                "debug_reason": debug_reason,  # <— helpful to show which one failed
            })
    ...
    # None dates last, latest first
    items.sort(key=lambda x: (x["dt"] is None, x["dt"] or datetime.min), reverse=False)
    items.reverse()
...
def governmentjobs(request):
    ...
    # 4) agar 20 ya 30 hi dikhane hain:
    latest_posts = mixed_objects[:6]
    return render(request, "jobs/governmentjobs.html", {"page_obj": page_obj, "latest_posts": latest_posts,})

def  jobsbyeducation(request):
    jobs = Job.objects.all().filter(status='published', is_active=True).order_by("-published_date")
    ...
    # 4) agar 20 ya 30 hi dikhane hain:
    latest_posts = mixed_objects[:11]
    return render(request, "jobs/jobsbyeducation.html", {"page_obj": page_obj, "latest_posts": latest_posts,})
...
def examresults(request):
    ...
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, "results/examresult.html", {"page_obj": page_obj})
...
def education(request):
    ...
    return render(request, "news/education.html", {
        "page_obj": page_obj,
        "items": page_obj.object_list,
        "latest_posts": latest_posts,
    })
...
def todaynews(request):
    ...
    return render(request, "news/todaynews.html", {
        "page_obj": page_obj,
        "items": page_obj.object_list, "latest_posts": latest_posts, "latest_results":latest_results, "latest_admitcards": latest_admitcards
    })
...
def indiaresults(request):
    ...
    return render(request, "news/indiaresults.html", {
        "page_obj": page_obj,
        "items": page_obj.object_list,
        "latest_posts": latest_posts,
    })
...
def notification_detail(request, slug):
    ...
    return render(request, "pdf/notification_detail.html", {"notification": notification})


def notification_list(request):
    # Sirf wahi jinke paas PDF hai, latest sabse upar
    notifications = (
        Notification.objects
        .filter(pdf_file__isnull=False)
        .exclude(pdf_file__exact="")
        .order_by("-created_at")
    )

    notifications = Notification.objects.all()
    mixed_objects = list(chain(notifications, ))
    # 3) sabko created_at ke hisaab se sort kar do (naya sabse upar)
    mixed_objects = sorted(
        mixed_objects,
    ...
    # 4) agar 20 ya 30 hi dikhane hain:
    latest_posts = mixed_objects[:5]
    return render(request, "pdf/notification_list.html", {"notifications": notifications,  "latest_posts": latest_posts,})
