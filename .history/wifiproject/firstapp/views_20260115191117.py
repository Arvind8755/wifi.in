...
def job_detail(request, slug):
    ...
    latest_jobs = Job.objects.filter(status='published', is_active=True).order_by("-published_date")[:16]  # latest 10 job
    latest_results = Result.objects.filter(status='published', is_active=True).order_by("-published_date")[:22]  # latest 10 job
    latest_admitcards = Admitcard.objects.filter(status='published', is_active=True).order_by("-published_date")[:22]  # latest 10 job
...
def html_sitemap(request):
  jobs = Job.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:30]
  results = Result.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:30]
  admitcards = Admitcard.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:30]
  govtupdates = Govtupdate.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:30]
  return render(request, 'mainfile/sitemap.html',{'jobs':jobs, 'results': results, 'admitcards': admitcards, 'govtupdates': govtupdates})
...
def admitcard_detail(request, slug):
    ...
    latest_jobs = Job.objects.filter(status='published', is_active=True).order_by("-published_date")[:16]  # latest 5 job
    latest_results = Result.objects.filter(status='published', is_active=True).order_by("-published_date")[:22]  # latest 5 job
    latest_admitcards = Admitcard.objects.filter(status='published', is_active=True).order_by("-published_date")[:22]  # latest 5 job
...
def govt_update_detail(request, slug):
  ...
  latest_jobs = Job.objects.filter(status='published', is_active=True).order_by("-published_date")[:16]  # latest 5 job
  latest_results = Result.objects.filter(status='published', is_active=True).order_by("-published_date")[:22]  # latest 5 job
  latest_admitcards = Admitcard.objects.filter(status='published', is_active=True).order_by("-published_date")[:]  # latest 5 job
...
def rajeshvari(request):
  ...
  qs = Job.objects.filter(status='published', is_active=True).filter(
        Q(category__icontains="rajeshvari") | Q(category__icontains="rajeshvari")
    ).order_by("-published_date")
  paginator = Paginator(qs, 22, orphans=1) 
  page_number = request.GET.get('page')
  page_obj = paginator.get_page(page_number)
  return render(request, 'author/Rajeshvari.html',  {"posts": qs, 'page_obj': page_obj, "latest_results": latest_results, "latest_admitcards": latest_admitcards})
...
def sakshi(request):
  ...
  qs = Govtupdate.objects.filter(status='published', is_active=True).filter(
        Q(category__icontains="sakshi") | Q(category__icontains="sakshi")
    ).order_by("-published_date")
  paginator = Paginator(qs, 22, orphans=1) 
  page_number = request.GET.get('page')
  page_obj = paginator.get_page(page_number)
  return render(request, 'author/sakshi.html',{"posts": qs, 'page_obj': page_obj, "latest_results": latest_results, "latest_admitcards": latest_admitcards})
