...
def html_sitemap(request):
  jobs = Job.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:3]
  results = Result.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:3]
  admitcards = Admitcard.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:3]
  govtupdates = Govtupdate.objects.all().filter(status='published', is_active=True).order_by("-published_date")[:3]
